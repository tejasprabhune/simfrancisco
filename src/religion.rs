...
pub fn assign(rec: &PumsRecord, rng: &mut impl Rng) -> Religion {
    // baseline weights (Pew SF metro), order matches Religion::all()
    let mut w = [0.24, 0.19, 0.02, 0.42, 0.02, 0.01, 0.03, 0.04, 0.03];
...
mod tests {
    ...
    fn deterministic_given_seed() {
        let r = rec(1, 1, 30);
        let a = assign(&r, &mut ChaCha8Rng::seed_from_u64(5));
        let b = assign(&r, &mut ChaCha8Rng::seed_from_u64(5));
        assert_eq!(a, b);
    }
    ...
    fn hispanic_skews_catholic_in_aggregate() {
        ...
        let mut cath_w = 0;
        for s in 0..2000u64 {
            let h = assign(&rec(8, 2, 40), &mut ChaCha8Rng::seed_from_u64(s));
            let w = assign(&rec(1, 1, 40), &mut ChaCha8Rng::seed_from_u64(s));
    ...
    fn unaffiliated_is_plurality_overall() {
        ...
        for s in 0..3000u64 {
            let r = rec(1, 1, 35);
            if assign(&r, &mut ChaCha8Rng::seed_from_u64(s)) == Religion::Unaffiliated {
                unaff += 1;
            }
        }
